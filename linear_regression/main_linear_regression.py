...
'''
@author: huangyong
algorithms for paper grouping desperated

this file try to use linear regression to fit the linear decrease of citation distribution, but it is hard to fit accurately.
...
def grouping_papers(citation_list,distribution_path,medium,step,isPercent=True):
    ...
    ax01 = fig.add_subplot(5,3,2)
    logging.info('plot ccdf ...')
    plot_ccdf(ax01,xs,ys)
...
def plot_ccdf(ax,xs,ys):

    ccdf_ys = []

    for  i,y in enumerate(ys):
        ccdf_ys.append(1-np.sum(ys[0:i]))

    ax.plot(xs,ccdf_ys,linewidth=2)
...
def main():
    ...
    parser.add_argument('-s','--step',dest='step',default=2,type=int,help='the step of grid search, default is 5.')
